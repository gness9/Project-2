...
struct thread
  {
    ...
    struct list child_list;    /* List containing each child process. */
    int exit_status;                   /* Stores the status upon exit */
    struct list_elem celem;       /* Used to keep track of the element in the child list. */
    struct semaphore hold;  /* Used to put a parent thread to sleep when it needs to wait for a child. */
